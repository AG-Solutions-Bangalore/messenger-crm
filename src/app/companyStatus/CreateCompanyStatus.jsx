...
const CreateCompanyStatus = () => {
      ...
      const handleSubmit = async () => {
        ...
        try {
          ...
          const response = await axios.post(`${BASE_URL}/api/panel-create-company-status`, formData, {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {pathname === "/company-status" ? (
              ...
              >
                <SquarePlus className="h-4 w-4 " /> Company Status
              </Button>
      ...
      <PopoverContent className="md:w-80">
        <div className="grid gap-4">
          <div className="space-y-2">
            <h4 className="font-medium leading-none">Create New Company Status</h4>
            <p className="text-sm text-muted-foreground">
              Enter the details for the new Company Status
            </p>
          </div>
          <div className="grid gap-2">
            <Input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, companyStatus: e.target.value }))
              }
            />
